...
void setup() {
  ...
  // 12.7 215 1.1
  anglePID.SetTunings(12.7, 235, 1.2);
...
void loop(){
  ...
  */
  ...
  float go = angleOutput+40;
if(angleInput > angleSetpoint+1.7 || angleInput < angleSetpoint-1.7){
  motorController.move(go);
  ...
  Serial.println(go);
     ...
     motorController.move(0*go/abs(go));
  }
  /*
  if(angleInput > angleSetpoint+1 || angleInput < angleSetpoint-1){

    if (go > 0) {
      motorSpeed = map(go, 0, 100, 0, 255);
    } 
    else {
      motorSpeed = map(go, -100, 0, -255, 0);
    }
    
    motorController.move(motorSpeed);
  }
  else{
     motorController.move(30*go/abs(go));
  }
*/
